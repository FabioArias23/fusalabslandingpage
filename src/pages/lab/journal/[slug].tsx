...
import type { JournalEntry } from "../../../components/lab/types";
...
function JournalEntryContent({ entry }: JournalPageProps) {
      ...
      <div className="max-w-[900px] mx-auto px-6 md:px-12">
        ...
        <header className="mb-12">
          ...
          <div className="flex items-center gap-4 text-[10px] font-conthrax tracking-widest text-white/20 uppercase">
            ...
            <span>{lang === "es" ? "Journal" : "Journal"}</span>
          </div>
        </header>
...
export default function JournalEntryPage({ entry }: JournalPageProps) {
  return (
    <LanguageProvider>
      <Layout>
        <JournalEntryContent entry={entry} />
      </Layout>
    </LanguageProvider>
  );
}
...
export const getStaticPaths: GetStaticPaths = async () => {
  const journalEs = labData.es.journal;
  const paths = journalEs.map((entry) => ({
    params: { slug: entry.id.replace("journal-", "") },
  }));
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const entryEs = labData.es.journal.find((e) => e.id === id);
  const entryEn = labData.en.journal.find((e) => e.id === id);
