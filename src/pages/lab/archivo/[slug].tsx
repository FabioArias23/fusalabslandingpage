...
import type { ArchivoEntry } from "../../../components/lab/types";
...
function ArchivoEntryContent({ entry }: ArchivoPageProps) {
      ...
      <div className="max-w-[900px] mx-auto px-6 md:px-12">
        ...
        <header className="mb-12">
          <div className="flex items-center gap-4 mb-6 flex-wrap">
            <span className="text-[10px] font-conthrax tracking-widest uppercase text-orange-400 border border-orange-400/20 px-3 py-1 bg-orange-500/5">
        ...
        <div className="p-6 bg-orange-500/5 border border-orange-500/20 rounded-sm mb-12">
          <h3 className="text-[10px] font-conthrax text-orange-400 uppercase tracking-widest mb-3">
            {lang === "es" ? "Razón del archivo" : "Reason for archiving"}
          </h3>
        ...
        <div className="mt-16 pt-8 border-t border-white/10">
          ...
          >
            <ArrowLeft size={14} />
            {lang === "es" ? "Ver todo el archivo" : "View full archive"}
          </Link>
        </div>
      </div>
    </div>
...
export default function ArchivoEntryPage({ entry }: ArchivoPageProps) {
  return (
    <LanguageProvider>
      <Layout>
        <ArchivoEntryContent entry={entry} />
      </Layout>
    </LanguageProvider>
  );
}
...
export const getStaticPaths: GetStaticPaths = async () => {
  const archivoEs = labData.es.archivo;
  const paths = archivoEs.map((entry) => ({
    params: { slug: entry.id.replace("arch-", "") },
  }));
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const entryEs = labData.es.archivo.find((e) => e.id === id);
  const entryEn = labData.en.archivo.find((e) => e.id === id);
