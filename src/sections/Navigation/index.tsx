  ...
  return (
    <nav
      className={`fixed top-0 w-full z-[90] transition-all duration-700 ${
        isScrolled
          ? "py-4 bg-fusa-black/80 backdrop-blur-2xl border-b border-fusa-indigo/50 shadow-[0_5px_30px_rgba(28,5,142,0.35)]"
          : "py-6 lg:py-8 border-b border-transparent"
      }`}
    >
        ...
        <div className="lg:hidden mt-6 flex justify-center items-center gap-6 sm:gap-10 border-t border-white/5 pt-5 text-[9px] sm:text-[11px] font-bold tracking-[0.2em] text-fusa-white/30 uppercase font-conthrax">
          {menuItems.map((item) => (
            <Link
              key={item}
              href={routeMap[item] || "/"}
              className="hover:text-fusa-white active:text-fusa-indigo transition-all"
              onMouseEnter={playHoverSound}
            >
              {item}
            </Link>
