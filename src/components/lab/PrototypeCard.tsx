...
import type { PrototipoEntry } from "./types";

const MATURITY_LABELS: Record<PrototipoEntry["maturity"], string> = {
  prototipo: "Prototipo",
  beta: "Beta",
  stable: "Stable",
};
...
interface Props {
  entry: PrototipoEntry;
  lang: "es" | "en";
}

export function PrototypeCard({ entry, lang }: Props) {
  ...
  return (
    ...
    >
      ...
      <p className="text-white/40 leading-relaxed mb-4">{entry.description}</p>
      <p className="text-[10px] font-conthrax tracking-widest text-white/20 uppercase mb-6">
        â†’ {entry.nextSteps}
      </p>
